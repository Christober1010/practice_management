<?php
/**
 * Google Drive Integration Status Check for Mahaverse (TEST ENVIRONMENT)
 * 
...
if (!$status['environment_vars']['GOOGLE_DRIVE_ENABLED']['set'] || !$driveEnabled) {
    $status['recommendations'][] = 'Set GOOGLE_DRIVE_ENABLED=true in backend-test/.env';
}

if (!$status['environment_vars']['GOOGLE_DRIVE_AUTH_MODE']['set']) {
    $status['recommendations'][] = 'Set GOOGLE_DRIVE_AUTH_MODE=oauth in backend-test/.env';
}

if ($authMode === 'oauth') {
    if (!$status['environment_vars']['GOOGLE_DRIVE_OAUTH_CLIENT_ID']['set']) {
        $status['recommendations'][] = 'Set GOOGLE_DRIVE_OAUTH_CLIENT_ID in backend-test/.env';
    }
    if (!$status['environment_vars']['GOOGLE_DRIVE_OAUTH_CLIENT_SECRET']['set']) {
        $status['recommendations'][] = 'Set GOOGLE_DRIVE_OAUTH_CLIENT_SECRET in backend-test/.env';
    }
    if (!$status['environment_vars']['GOOGLE_DRIVE_OAUTH_REDIRECT_URI']['set']) {
        $status['recommendations'][] = 'Set GOOGLE_DRIVE_OAUTH_REDIRECT_URI in backend-test/.env';
    }
    if (!$status['environment_vars']['GOOGLE_DRIVE_TOKEN_ENCRYPTION_KEY']['set']) {
        $status['recommendations'][] = 'Set GOOGLE_DRIVE_TOKEN_ENCRYPTION_KEY in backend-test/.env (generate with: python3 -c "import os, base64; print(base64.b64encode(os.urandom(32)).decode())")';
    }
...
if (!$status['files']['drive_helper_local']['exists'] && !$status['files']['drive_helper_launchpad']['exists']) {
    $status['recommendations'][] = 'Copy drive_helper.php to backend-test/ folder or ensure maha-launchpad/backend/drive_helper.php exists';
}
...
if (!$status['environment_vars']['GOOGLE_DRIVE_ROOT_FOLDER_ID']['set']) {
    $status['recommendations'][] = 'Set GOOGLE_DRIVE_ROOT_FOLDER_ID in backend-test/.env (get from Google Drive folder URL)';
}
