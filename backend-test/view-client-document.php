<?php
/**
 * View Client Document from Google Drive (backend-test)
 */
...
try {
    ...
    header('Content-Disposition: inline; filename="' . addslashes($metadata['name'] ?? 'document') . '"');
    header('Content-Length: ' . strlen($fileContent));
    header('Cache-Control: private, max-age=3600');

    echo $fileContent;
} catch (Exception $e) {
    error_log("Error in view-client-document.php: " . $e->getMessage());
    http_response_code(500);
    header('Content-Type: application/json');
