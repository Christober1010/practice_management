-- RBAC: permissions catalog + Mahaverse role grants (no changes to existing user tables).
-- For Launchpad database, run the same CREATE + permission INSERTs, then run
-- app/launchpad/backend/seed_rbac_launchpad_grants.sql
...
CREATE TABLE IF NOT EXISTS rbac_role_grants (
  ...
  CONSTRAINT fk_rbac_perm FOREIGN KEY (permission_id) REFERENCES rbac_permissions(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
('view.reports', 'Reports (screen)', 'mahaverse_view', 'mahaverse', 180),
...
-- Mahaverse role grants (idempotent: replace these roles)
DELETE FROM rbac_role_grants WHERE role_name IN ('admin','bcba','rbt','biller','parent');

INSERT INTO rbac_role_grants (role_name, permission_id)
SELECT 'admin', id FROM rbac_permissions WHERE app_scope = 'mahaverse';

INSERT INTO rbac_role_grants (role_name, permission_id)
SELECT r.role, p.id FROM (
  SELECT 'bcba' AS role UNION SELECT 'rbt'
) r
...
INSERT INTO rbac_role_grants (role_name, permission_id)
SELECT 'biller', id FROM rbac_permissions WHERE perm_key IN (
...
INSERT INTO rbac_role_grants (role_name, permission_id)
SELECT 'parent', id FROM rbac_permissions WHERE perm_key IN (
