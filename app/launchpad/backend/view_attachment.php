...
// These headers are set by config.php and would block iframe embedding
...
// Require authentication - allow token from query parameter for iframe embedding
// This is needed because iframes don't send Authorization headers
$authUser = null;
...
// Last resort: try token from query parameter (for iframe embedding)
...
try {
    ...
    if ($isReader) {
        ...
        throw new Exception("Access denied. You can only view attachments for staff you created.");
    }
    ...
    // IMPORTANT: Remove X-Frame-Options that config.php sets, as it blocks iframe embedding
    header_remove('X-Frame-Options');
    header_remove('Content-Type');
    header_remove('X-Content-Type-Options');
    header_remove('X-XSS-Protection');
    ...
    header('Content-Disposition: inline; filename="' . addslashes($attachment['original_filename']) . '"', true);
    ...
    // Use Content-Security-Policy to allow iframe embedding from allowed origins
    // This is more flexible than X-Frame-Options and allows cross-origin embedding
    $cspFrameAncestors = "'self'";
    foreach ($allowedOrigins as $allowed) {
        $cspFrameAncestors .= ' ' . $allowed;
    }
    header("Content-Security-Policy: frame-ancestors $cspFrameAncestors", true);
