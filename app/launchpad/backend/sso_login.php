...
//   Header (recommended): X-SSO-Secret: <shared-secret>
//
// Response:
...
if ($expectedSecret !== '') {
    ...
    $provided = '';
    if (isset($headers['X-SSO-Secret'])) $provided = $headers['X-SSO-Secret'];
    elseif (isset($headers['x-sso-secret'])) $provided = $headers['x-sso-secret'];
    elseif (isset($_SERVER['HTTP_X_SSO_SECRET'])) $provided = $_SERVER['HTTP_X_SSO_SECRET'];
...
// Mint token (raw token returned to client; hash stored in AuthTokens)
$rawToken = bin2hex(random_bytes(32)); // 64 hex chars
$tokenHash = hash('sha256', $rawToken);
$expiresAt = date('Y-m-d H:i:s', strtotime('+24 hours'));

// Ensure AuthTokens table exists (best-effort)
$conn->query("
