  ...
  const refreshSessions = useCallback(async () => {
    ...
    try {
      ...
      const mapped = rows.map((row) => {
        ...
        return {
          sessionId: row.session_id || `temp-${Math.random().toString(36).substring(2)}`,
    ...
    try {
      const resp = await fetch(`${baseUrl}/add-session.php`, {
        ...
        body: JSON.stringify({
          ...
          cancelledReason: sessionToDelete.cancelledReason || "Session cancelled",
        }),
        cache: "no-store",
        mode: "cors",
      })
      ...
      if (data.success) {
        ...
        toast.success(`Session${data.rows_affected > 1 ? "s" : ""} cancelled successfully!`)
