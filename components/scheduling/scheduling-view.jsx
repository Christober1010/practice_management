...
import { CalendarIcon, Plus, ChevronLeft, ChevronRight, Edit, Eye, Clock, List, Trash2 } from "lucide-react"
...
function isSameLocalDay(aStr, bDate) {
  ...
  const local = new Date(aStr)
  return sameDay(local, bDate)
}

function getVisualStatus(session, todayDate) {
  const isCancelled =
    (session.status || "").toLowerCase() === "cancelled" || (session.STATUS || "").toLowerCase() === "cancelled"
  ...
  const dayOnlySession = new Date(sessionDay.getFullYear(), sessionDay.getMonth(), sessionDay.getDate())
  const dayOnlyToday = new Date(todayDate.getFullYear(), todayDate.getMonth(), todayDate.getDate())
...
export default function SchedulingView() {
  ...
  const uniqueLocations = useMemo(() => {
    const locations = new Set(sessions.map((session) => session.locationAddress).filter(Boolean))
    return ["All", ...Array.from(locations)]
  }, [sessions])

  const uniqueProviders = useMemo(() => {
    const providers = new Set(sessions.map((session) => session.provider_name).filter(Boolean))
    return ["All", ...Array.from(providers)]
  }, [sessions])

  const uniqueClients = useMemo(() => {
    const clients = new Set(sessions.map((session) => session.clientName).filter(Boolean))
    return ["All", ...Array.from(clients)]
  }, [sessions])

  const filteredSessions = useMemo(() => {
    return sessions.filter((session) => {
      const matchesLocation = locationFilter === "All" || !locationFilter || session.locationAddress === locationFilter
      const matchesStaff = staffFilter === "All" || !staffFilter || session.provider_name === staffFilter
      const matchesClient = clientFilter === "All" || !clientFilter || session.clientName === clientFilter
  ...
  const calendarData = useMemo(() => {
    if (viewMode === "today") {
      ...
      return {
        days,
        title: `${formatDateLabel(weekStart)} - ${formatDateLabel(endOfWeek(currentDate))}`,
      }
    } else {
  ...
  const refreshSessions = useCallback(async () => {
    ...
    try {
      ...
      const mapped = rows.map((row) => {
        ...
        const startMs = startUtc ? Date.parse(startUtc.replace(" ", "T") + "Z") : Number.NaN
        const endMs = endUtc ? Date.parse(endUtc.replace(" ", "T") + "Z") : Number.NaN
        ...
        return {
          sessionId: row.session_id || `temp-${Math.random().toString(36).substring(2)}`, // Fallback ID
          ...
          scheduledHours: row.scheduled_hours != null ? Number(row.scheduled_hours) : computedScheduled,
          renderedHours: row.rendered_hours != null ? Number(row.rendered_hours) : 0,
        }
      })
      ...
      const inRange = (dtStr) => {
        ...
        return d >= dateRange.start && d <= new Date(dateRange.end.getTime() + 24 * 60 * 60 * 1000)
      }
  ...
  const confirmDeleteSession = async () => {
    ...
    try {
      const resp = await fetch(`${baseUrl}/add-session.php?id=${sessionToDelete.sessionId}`, {
        method: "DELETE",
        headers: {
  ...
  const getCellSessions = (dateObj) => {
    if (!dateObj) return []
    return filteredSessions // Use filteredSessions for all views
      .filter((session) => session?.startDateTime && isUTCStringOnDate(session.startDateTime, dateObj))
      .sort(
        (a, b) =>
  ...
  function layoutDaySessionsForTimeline(sessions, userTimezone, expandedSessionId = null, expandExtraPx = 0) {
    ...
    if (expandedSessionId && expandExtraPx > 0) {
      const expandedItem = layouts.find((item) => item.session.sessionId === expandedSessionId)
      if (expandedItem) {
        ...
        <div className="flex border border-slate-300 rounded-md overflow-hidden bg-white">
            ...
            <div className="relative" style={{ height: `${timelineHeight}px` }}>
              {timeSlots.map((slot, idx) => {
                ...
                return (
                  ...
                  >
                    {shouldShowLabel && (
                      <span className={`text-xs ${slot.isHour ? "text-gray-700 font-medium" : "text-gray-500"}`}>
          ...
          <div className="flex-1 relative">
            ...
            <div className="h-12 border-b border-slate-300 bg-slate-50 flex items-center justify-between px-4">
              <span className="font-semibold text-slate-700">{formatDateLabel(currentDate)}</span>
              <Button
                variant="outline"
                size="sm"
            ...
            <div className="relative" style={{ height: `${timelineHeight}px` }}>
              {timeSlots.map((slot, idx) => {
                ...
                return shouldShowLine ? (
                  <div key={idx} className="absolute w-full" style={{ top: `${topPosition}px` }}>
                    <div
                      className={`w-full ${
                        slot.isHour ? "border-t border-slate-300" : "border-t border-slate-300 border-dashed"
                      }`}
                    />
                  </div>
                ) : null
              ...
              {layouts.map((layout) => {
                ...
                const startLabel = formatTime12hFromUTC(s.startDateTime, userTimezone)
                const endLabel = formatTime12hFromUTC(s.endDateTime, userTimezone)
                ...
                const heightPx = Math.max(durationMinutes * pixelsPerMinute, 30)

                // Replace the hardcoded teal classes with dynamic color classes
                    ...
                    <div className="p-3 h-full flex flex-col">
                      ...
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex-1 min-w-0">
                          ...
                          {s.provider_name && <p className="text-xs text-gray-600">Provider: {s.provider_name}</p>}

                          {s.supervising_provider_name && (
                            <p className="text-xs text-gray-500">Supervisor: {s.supervising_provider_name}</p>
                          )}
                          {s.locationAddress && <p className="text-xs text-gray-500">Location: {s.locationAddress}</p>}
                          {s.authCode && <p className="text-xs text-gray-500">Auth Code: {s.authCode}</p>}
                          {s.quickNote && (
                            <p className="text-xs text-gray-500 italic border-t pt-2 mt-1">Note: {s.quickNote}</p>
                          )}
                        </div>
        ...
        <div className="flex border border-slate-300 rounded-md overflow-hidden bg-white">
          ...
          <div className="flex-1 flex">
            {days.map((dateObj, dayIdx) => {
              ...
              const { layouts } = layoutDaySessionsForTimeline(daySessions, userTimezone, null, 0)

              const dayName = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][dateObj.getDay()]

              return (
                <div key={dayIdx} className="flex-1 border-r border-slate-300 last:border-r-0">
                  {/* Day header */}
                  <div
                  ...
                  >
                    ...
                    <span className={`text-sm font-semibold ${isToday ? "text-teal-600" : "text-slate-700"}`}>
                      {dateObj.getDate()}
                    </span>
                  </div>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            height: `${heightPx}px`,
                            minHeight: `${heightPx}px`,
                            zIndex: hoveredSession?.sessionId === s.sessionId ? 9999 : 1,
                          }}
                        >
                          <div className="p-2 h-full flex flex-col text-xs">
                            <div className="font-semibold truncate capitalize">{s.clientName || "Unknown"}</div>
                            <div className="text-[10px] opacity-90 truncate">
                              {formatTime12hFromUTC(s.startDateTime, userTimezone)} -{" "}
                              {formatTime12hFromUTC(s.endDateTime, userTimezone)}{" "}
                            </div>
                            <div className="text-[10px] opacity-90 truncate">{s.provider_name}</div>
                          </div>
                          {hoveredSession?.sessionId === s.sessionId && (
                            <div className="absolute top-1/2 z-[9999999] bg-white shadow-lg p-3 rounded-lg border left-0 right-0 mt-2 pointer-events-auto w-64">
                              <div className="flex flex-col gap-2">
                                <p className="font-semibold text-gray-800 capitalize text-sm">{s.clientName}</p>
                                ...
                                <p className="text-sm text-gray-600">Provider: {s.provider_name}</p>
                                {s.supervising_provider_name && (
                                  <p className="text-xs text-gray-500">Supervisor: {s.supervising_provider_name}</p>
                                )}
                                {s.locationAddress && (
                                  <p className="text-xs text-gray-500">Location: {s.locationAddress}</p>
                                )}
                                {s.authCode && <p className="text-xs text-gray-500">Auth Code: {s.authCode}</p>}
                                {s.quickNote && (
                                  <p className="text-xs text-gray-500 italic border-t pt-2 mt-1">Note: {s.quickNote}</p>
          ...
          <div className="hidden md:grid md:grid-cols-7 gap-2">
            {arr.map((dateObj, idx) => {
              ...
              const sortedSessions = daySessions.sort(
                (a, b) =>
                  toMinutes12h(
                    formatTime12hFromUTC(a.startDateTime, a.startTZ), // Use a.startTZ
                  ) -
                  toMinutes12h(
                    formatTime12hFromUTC(b.startDateTime, b.startTZ), // Use b.startTZ
                  ),
              )
              return (
                <Card
                    ...
                    <div className="flex justify-between mb-1">
                      <span
                        className={`text-sm font-medium flex items-center ${
                          isToday ? "text-teal-600" : isPast ? "text-muted-foreground" : "text-foreground"
                        }`}
                      >
                        {dateObj.getDate()}
                        {isToday && <Badge className="ml-2 text-[10px] py-0.5 px-2">Today</Badge>}
                      </span>
                      <Button
                        variant="ghost"
                        size="sm"
                    ...
                    <div className="space-y-1 relative">
                      {sortedSessions.length > 0 ? (
                        sortedSessions.map((session) => {
                          const startLabel = formatTime12hFromUTC(session.startDateTime, userTimezone)
                            .replace(":00 ", "")
                            .replace(" UTC", "")
                          // Apply dynamic color classes
                          ...
                          return (
                            <div
                              key={session.sessionId || `${session.clientId}-${session.startDateTime}`}
                                ...
                                <div className="absolute z-[1000] bg-white shadow-lg p-3 rounded-lg border top-full left-0 right-0 mt-2 pointer-events-auto w-56">
                                  <div className="flex flex-col gap-1">
                                    ...
                                    <p className="text-sm text-gray-600">
                                      {formatTime12hFromUTC(session.startDateTime, userTimezone)} -{" "}
                                      {formatTime12hFromUTC(session.endDateTime, userTimezone)}
                                    </p>
                                    <p className="text-sm text-gray-600">{session.provider_name}</p>
                                    {session.locationAddress && (
                                      <p className="text-xs text-gray-500">{session.locationAddress}</p>
                                    )}
                                    {session.quickNote && (
                                      <p className="text-xs text-gray-500 italic">{session.quickNote}</p>
                                    )}
                                  </div>
                                </div>
                              )}
              ...
              return (
                <Card
                  ...
                  className={`p-3 cursor-pointer transition-all border border-gray-200 overflow-visible ${
                    isPast ? "opacity-60" : ""
                  } ${isSelected ? "ring-2 ring-teal-500" : ""} ${isToday ? "bg-teal-100" : ""}`}
                  onClick={() => handleDayClick(dateObj)}
                >
                          ...
                          >
                            ...
                            {hoveredSession?.sessionId === session.sessionId && (
                              <div className="absolute z-[9999] bg-white shadow-xl p-3 rounded-lg border left-0 right-0 top-full mt-2 pointer-events-none">
                                <div className="flex flex-col gap-1">
                                  <p className="font-semibold text-gray-800 capitalize">{session.clientName}</p>
                                  <p className="text-sm text-gray-600">
                                    {formatTime12hFromUTC(session.startDateTime, userTimezone)} -{" "}
                                    {formatTime12hFromUTC(session.endDateTime, userTimezone)}
                                  </p>
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage appointments and therapy sessions</p>
        </div>
        <Button
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
              <div className="flex items-center lg:justify-start justify-center w-full gap-2">
                <Button
                  ...
                  className={viewMode === "today" ? "bg-teal-600 hover:bg-teal-700 text-white" : ""}
                >
                ...
                <Button
                  ...
                  className={viewMode === "week" ? "bg-teal-600 hover:bg-teal-700 text-white" : ""}
                >
                ...
                <Button
                  ...
                  className={viewMode === "month" ? "bg-teal-600 hover:bg-teal-700 text-white" : ""}
                >
              ...
              <div className="flex items-center lg:justify-end justify-center w-full gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-[200px] text-center">
                    <h3 className="font-semibold text-foreground">{calendarData.title}</h3>
                  </div>
