...
import AddServiceCodeModal from "./add-service-code-modal";
import DeleteConfirmModal from "../master-data/DeleteConfirmModal";

export default function ServiceCodeSetup() {
  ...
  const loadServiceCodes = async () => {
    try {
      ...
      if (data?.success) {
        ...
        toast.error(data?.message || "Failed to load service codes");
      }
    } catch (err) {
      console.error(err);
  ...
  const handleAddServiceCode = async (code) => {
    try {
      ...
      if (result.success) {
        toast.success(editingCode ? "Service code updated!" : "Service code saved!");
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-800">Service Codes</h2>
          <p className="text-slate-600">Manage service code master data</p>
        </div>
        <div className="flex gap-3">
          ...
          >
            <Plus className="h-4 w-4 mr-2" /> Add Service Code
          </Button>
        </div>
      </div>
              ...
              <Table>
                ...
                <TableBody>
                  {displayedCodes.map((code) => (
                    <TableRow key={code.code_id} className="hover:bg-slate-50">
                      <TableCell className="font-medium p-2">
                        {code.code}
                      </TableCell>
                      <TableCell className="p-2">{code.code_description}</TableCell>
