...
Prod `recurring_id` gap was missed because API tests **skipped** on `Unknown column` and UI scheduling never **submits** add-session. Added `e2e/00-schema-readiness.spec.ts` + `assertNoSchemaDrift` — prod API now **fails** on migration drift. After `20260606_130846_sessions_add_recurring_id.sql`: `pnpm run test:e2e:api:prod` → 24 passed, 1 skipped (no client activities for trial POST).
