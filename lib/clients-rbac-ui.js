...
/**
 * Edit client: clients.update OR legacy clients.write
 * (Archive alone must not unlock edit.)
 */
export function allowsClientUpdate(canAny) {
  return canAny([PERM.CLIENTS_UPDATE, PERM.CLIENTS_WRITE]);
}

/**
 * Archive/restore: clients.archive OR legacy clients.write
 * (Edit alone must not unlock archive.)
 */
export function allowsClientArchive(canAny) {
  return canAny([PERM.CLIENTS_ARCHIVE, PERM.CLIENTS_WRITE]);
}
