...
export const NAV_VIEW_ACCESS_PAIRS = [
  ...
  { nav: PERM.NAV_SCHEDULING, view: PERM.VIEW_SCHEDULING, title: "Appointments" },
  ...
  { nav: PERM.NAV_REPORTS, view: PERM.VIEW_REPORTS, title: "Reports hub" },
...
export function normalizeNavViewKeys(keys) {
  ...
  // Data Collection sub-screens: manage key implies view key for sidebar/route guards.
  const manageViewPairs = [
    ["master_data.domains", "view.domains"],
    ["master_data.programs", "view.programs"],
    ["master_data.targets", "view.targets"],
    ["master_data.prompts", "view.prompts"],
    ["master_data.behavior_categories", "view.behavior_categories"],
    ["master_data.behaviors", "view.behaviors"],
  ];
  for (const [manage, view] of manageViewPairs) {
    if (set.has(manage) || set.has(view)) {
      set.add(manage);
      set.add(view);
    }
  }
  // Any individual report screen implies Reports hub (menu + screen).
  const reportScreens = [
  ...
  if (reportScreens.some((k) => set.has(k))) {
    set.add(PERM.NAV_REPORTS);
    set.add(PERM.VIEW_REPORTS);
  }
  return [...set];
}
