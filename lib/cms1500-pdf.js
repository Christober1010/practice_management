...
async function buildCms1500TemplatePdfBytes(payload) {
  ...
  /** Template exports Box 25 EIN toggle as checkbox field name "276". */
  let einMark = null;
  ...
  setText("ins_name", `${safe(insured.last_name)}, ${safe(insured.first_name)}`);
  ...
  setText("ins_street", joinAddress([insured.address_line_1, insured.address_line_2]));
  setText("ins_city", insured.city);
  setText("ins_state", insured.state);
  setText("ins_zip", insured.zip);
  ...
  lines.slice(0, 6).forEach((line, idx) => {
    ...
    setText(`diag${n}`, (line.diagnosis_pointers || []).join(","));
    setText(`day${n}`, line.units);
    setText(`local${n}`, line.authorization_number);
  });

  setText("tax_id", billing.tax_id);
  ...
  try {
    ...
    // Template variant without SSN toggle.
  }
  try {
    const einCb = form.getCheckBox("276");
    const ew = einCb.acroField.getWidgets();
    if (ew[0]) {
      einMark = checkboxXMarkParams(ew[0].getRectangle(), height, 9);
    }
    einCb.uncheck();
  } catch {
    // Missing EIN widget.
